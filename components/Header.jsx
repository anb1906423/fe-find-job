...
const Header = () => {
  ...
  const taiKhoan = [
    ...
    },
    {
      ...
      function: () => {
        Swal.fire({
          imageUrl: 'https://upload.wikimedia.org/wikipedia/commons/thumb/f/fd/Font_Awesome_5_solid_user-check.svg/768px-Font_Awesome_5_solid_user-check.svg.png?20180810222541',
          imageWidth: 100,
          imageHeight: 96,
          imageAlt: 'Custom image',
          title: 'Chọn loại tài khoản',
        ...
        <ul className="menu p-2">
          {
            menu.map((item, index) => {
              return (
                <li className="menu-item fw-bold text-uppercase" key={index}><a href={item.href}>{item.title}</a></li>
              )
            })
          }
        </ul>
        ...
        <ul className="tai-khoan p-2 ms-auto">
          {
            taiKhoan.map((item, index) => {
              return (
                <li onClick={item.function} className="tai-khoan-item menu-item fw-bold text-uppercase" key={index}>
                  <a href={item.href}>
                    {item.title}
                  </a>
                </li>
