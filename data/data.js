export const majors = [
    {
        label: '-- Tất cả chuyên ngành --',
        value: '-- Tất cả chuyên ngành --',
    },
    {
        label: 'Bán hàng / Kinh doanh',
        value: 'Bán hàng / Kinh doanh',
    },
    {
        label: 'Dịch vụ khách hàng / Tư vấn / CSKH',
    },
    {
        label: 'Kế toán / Kiểm toán',
    },
    {
        label: 'Khách sạn / Nhà hàng',
    },
    {
        label: 'Du lịch',
    },
    {
        label: 'CNTT - Phần mềm',
    },
    {
        label: 'CNTT - Phần cứng / Mạng',
    },
    {
        label: 'Bưu chính viễn thông',
    },
    {
        label: 'Giải trí',
    },
    {
        label: 'Thực phẩm / Đồ uống',
    },
    {
        label: 'Điện / Điện tử / Điện lạnh',
    },
    {
        label: 'Cơ khí / Ô tô / Tự động hóa',
    },
    {
        label: 'Sản xuất / Vận hành sản xuất',
    },
    {
        label: 'Bảo trì / Sữa chữa',
    },
    {
        label: 'Ngân hàng',
    },
    {
        label: 'Chứng khoán / Vàng / Ngoại tệ',
    },
    {
        label: 'Tài chính / Đầu tư',
    },
    {
        label: 'Hành chính / Thư ký / Văn phòng',
    },
    {
        label: 'Công chức / Viên chức / Cán bộ nhà nước',
    },
    {
        label: 'Giáo dục / Đào tạo / Thư viện',
    },
    {
        label: 'Nhân sự',
    },
    {
        label: 'Kho / Vật tư / Thu mua',
    },
    {
        label: 'Vận chuyển / Tài xế',
    },
    {
        label: 'Công nhân / Lao động phổ thông',
    },
    {
        label: 'Khu chế xuất / Khu công nghiệp',
    },
    {
        label: 'Dệt may / Da giày',
    },
    {
        label: 'An ninh / Bảo vệ',
    },
       
    //  'Y tế / Chăm sóc sức khỏe', 'Công nghệ thực phẩm / Dinh dưỡng', 'Dược / Công nghệ sinh học', 'Hóa học',
    // 'Môi trường / Xử lý chất thải', 'Chăn nuôi / Thú y', 'Thủy sản / Hải sản', 'Nông / Lâm / Ngư nghiệp', 'Mỹ phẩm / Thời trang / Trang sức',
    // 'Làm đẹp', 'Thể dục / Thể thao', 'Phiên dịch', 'Ngoại thương / Xuất nhập khẩu', 'Bất động sản', 'Xây dựng', 'Kiến trúc / Nội ngoại thất',
    // 'Mỹ thuật', 'Nghệ thuật / Điện ảnh', 'Marketing / Truyền thông / Quảng cáo', 'Báo chí / Biên tập', 'Quản lý chất lượng (QA, QC)',
    // 'Luật / Pháp lý', 'Giao thông / Vận tải / Thủy lợi', 'Hàng không / Hàng hải', 'Dầu khí / Hóa chất', 'Bảo hiểm', 'Khác'
]

export const provinces = [
    '-- Tất cả địa điểm --', 'An Giang', 'Bà Rịa - Vũng Tàu', 'Bắc Giang', 'Bắc Kạn',
    'Bạc Liêu', 'Bắc Ninh', 'Bến Tre', 'Bình Định', 'Bình Dương', 'Bình Phước', 'Bình Thuận', 'Cà Mau',
    'Cần Thơ', 'Cao Bằng', 'Đà Nẵng', 'Đắk Lắk', 'Đắk Nông', 'Điện Biên', 'Đồng Nai', 'Đồng Tháp',
    'Gia Lai', 'Hà Giang', 'Hà Nam', 'Hà Nội', 'Hà Tĩnh', 'Hải Dương', 'Hải Phòng', 'Hậu Giang', 'Hòa Bình',
    'Hưng Yên', 'Khánh Hòa', 'Kiên Giang', 'Kon Tum', 'Lai Châu', 'Lâm Đồng', 'Lạng Sơn', 'Lào Cai', 'Long An',
    'Nam Định', 'Nghệ An', 'Ninh Bình', 'Ninh Thuận', 'Phú Thọ', 'Phú Yên', 'Quảng Bình', 'Quảng Nam', 'Quảng Ngãi',
    'Quảng Ninh', 'Quảng Trị', 'Sóc Trăng', 'Sơn La', 'Tây Ninh', 'Thái Bình', 'Thái Nguyên', 'Thanh Hóa',
    'Thừa Thiên Huế', 'Tiền Giang', 'TP Hồ Chí Minh', 'Trà Vinh', 'Tuyên Quang', 'Vĩnh Long', 'Vĩnh Phúc', 'Yên Bái',  
]